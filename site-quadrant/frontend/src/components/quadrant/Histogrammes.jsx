...
//
// aria-hidden : information redondante avec les positions des bulles,
// pas indispensable au lecteur d'écran (le toggle suit le pattern
// « décoratif » du quadrant lui-même qui est aria-label="Quadrant").
...
export default function Histogrammes({ bulles }) {
  ...
  return (
    <g className="quadrant-histogrammes" aria-hidden="true">
      ...
      {countsY.map((c, i) => {
        if (c === 0) return null;
        const w = (c / maxY) * HISTO_BAND_TAILLE;
