...
La coloration s'applique à **toutes** les bulles, autorisées ou non.
...
L'API utilise ce filtre **différemment selon la vue** :

- **Vue Mentions** : filtrage strict en SQL — seules les mentions de l'établissement de contexte remontent. Les bulles affichées sont donc toutes autorisées.
  ...
  Avec `:motif` = `'%;<contexte_id>;%'` calculé en PHP. Les délimiteurs systématiques évitent les faux positifs.

- **Vue Établissements** : le filtre n'est **pas** appliqué en SQL — toutes les bulles de France remontent. Le `filtre_perimetre` de chaque bulle est en revanche conservé pour calculer, côté PHP, un drapeau `details_accessibles` par bulle :

  ```
  details_accessibles = (filtre_perimetre contient ;<contexte_id>;)
  ```

  Les bulles avec `details_accessibles = false` sont anonymisées dans la réponse JSON (libellé vide). Cf. section 4 — Groupe 2 — Établissements pour la justification métier.
