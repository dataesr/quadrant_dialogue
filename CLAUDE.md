...
- Lecture auto stable : le ref `millesimePrecedentRef` (lu par `setInterval` pour décider du prochain millésime) est désormais synchronisé **inconditionnellement** à la fin du useEffect de trace, y compris quand la logique de trace est sautée (comparer en cours, etc.). Avant ce fix, en vue Positionnement (`traceContinueEnabled` était à `false`), le ref n'était jamais mis à jour, le timer avançait au 2ᵉ millésime puis se figeait. La trace continue est désormais active dans les **deux** vues (80 bulles × 3 segments = ~240 polylines, coût négligeable).
