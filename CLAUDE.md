...
- `GraphiqueEvolutionSalaires.jsx` — SVG 3 courbes Q1/Q3 (gris) + Q2 (bleu `#0E0E60`, épais, points marqués), abscisses `millesimes`|`durees`, `marqueur_x` (anneau sur l'abscisse courante), `echelle_y` optionnelle (échelle commune), courbes interrompues sur trou.
...
**UX — barre latérale (fiche détaillée d'une mention).** Section `SectionSalaires` (`DetailsPanel.jsx`), affichée **seulement si `salaires.disponible`** (sinon absente, pas de placeholder). Placée **après les cards X/Y** (arbitrage : le brief disait « après le Taux de poursuivants », dont la position dépend de s'il est en axe ; après les indicateurs principaux est l'emplacement stable le plus proche). Trois blocs : (1) `BlocSalaireParcoursup` compact à la **durée effective** ; (2) évolution **sur les millésimes** (durée fixe) ; (3) évolution **sur la durée** (millésime fixe = courant). **Règle de durée effective** : durée d'un axe (X prioritaire, sinon Y) → sinon 12 → sinon première durée présente au millésime courant (12→18→24→30). **Échelle Y commune** aux blocs 2 et 3 (comparer variation interannuelle vs sur la durée). Note de lecture sous les blocs.

**UX — onglet « Salaires » de la modale d'analyse fine.** `OngletSalaires.jsx`, inséré après « Parcours » (avant « Mentions agrégées »), **visible seulement si au moins une sous-pop a des salaires** (donc masqué en mode établissement et pour les BUT). Grille de 6 blocs (référence + 5 sous-pops principales), chacun = `BlocSalaireParcoursup` + `GraphiqueEvolutionSalaires` sur les durées (échelle Y **individuelle**). Sous-pop sans données → bloc conservé avec message (grille préservée). Piloté par le **slider/animation commun** (`dureeCourante`) ; les contrôles ⏮▶⏭ + Vitesse sont **partagés** Quadrant ↔ Salaires (`renderControlesLecture(scope)`), et `changerOnglet` ne met plus en pause en passant sur Salaires. **Cas durée = 6** : bloc Parcoursup « Non disponible à 6 mois », graphique sur 12/18/24/30.
