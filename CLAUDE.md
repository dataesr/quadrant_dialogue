...
- **Frontend** : bouton « Analyse de l'insertion par sous-population » en bas de la fiche détaillée (`DetailsPanel.jsx`, deux vues), ouvre `ModaleAnalyseSousPopulations` (large ~90 % viewport, état local au panneau). 3 sections empilées : `RepartitionsPromo` (barres empilées 100 %, statiques), `TableauEcarts` (comparaison à la référence, écarts colorés par seuil ±5 pts, toggle « croisements simples », colonnes emploi animées), `MiniQuadrantSousPop` (mini-quadrant animé sur la durée d'observation, axes emploi salarié FR × emploi stable, bulles colorées par critère via `COULEUR_CRITERE_SOUS_POP`). La modale hérite du millésime du quadrant et d'une durée initiale (`dateInserX || dateInserY`), puis l'animation pilote la durée.
