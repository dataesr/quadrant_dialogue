...
**3. Compteur de mouvements de bulles — vue Mentions uniquement.** `CompteurMouvements.jsx` contextualise les mouvements entre le millésime affiché et le précédent. **Déplacé en Phase 15.4 dans la modale d'animation** (cf. ci-dessous) — il n'est plus affiché sous le quadrant principal statique (sa lecture n'avait de sens que pendant l'animation des transitions). Calculé côté **backend** à partir des points **bruts** (avant filtrage d'affichage : forme/diffusion, représentativité). Cohérent avec les filtres disciplinaires actifs.
...
Chaque catégorie expose la **liste des libellés** concernés. `comparaison_disponible=false` au premier millésime → « Première année observée — pas de comparaison ».

### Modale d'animation — alignement 50/50 + compteur déplacé (Phase 15.4)

**Alignement 50/50 (`ModaleAnimation.jsx`).** Le sélecteur **« Suivre une mention »** (gauche) et le **curseur de millésime** (droite) sont sur une **seule ligne** `.modale-animation-controles-haut` (flex row, deux colonnes `flex:1`, gap central 2rem, padding latéral 1.25rem). Les titres sont les **libellés natifs** des composants (`.fr-label` du `Combobox` « Suivre une mention » et du `SliderDuree` « Millésime : 2022 »), déjà homogènes. La colonne gauche reste **présente même en Positionnement** (vide, le sélecteur de mention n'y est pas applicable) → le curseur garde sa demi-largeur droite. Repli en 2 lignes sous 900 px. Le `SliderDuree` remplit sa colonne (`width:100%` au lieu des 400 px par défaut).

**Compteur déplacé dans la modale (`.modale-animation-compteur`).** Le compteur est rendu **sous le quadrant animé**, au-dessus des contrôles lecture/vitesse (encadré de fins filets). Données par millésime calculées par **`/quadrant/serie-temporelle`** (helper `calculerMouvements`, même convention de comptage que ci-dessus, seuil 20) sur les points **bruts** par millésime ; le « précédent » est le **millésime précédent de la série** (`millesime_precedent` renvoyé pour le libellé). Chaque entrée `series[millesime].mouvements` porte `comparaison_disponible`, `millesime_precedent`, `seuil` et les 4 listes. Le `/quadrant` (un seul millésime) **ne calcule plus** `mouvements` (retiré en 15.4). Le compteur se met à jour à chaque tick ; `key={millesimeCourant}` remonte le composant → léger **fondu d'apparition** CSS (opacité 0.35→1, 0.35 s, respecte `prefers-reduced-motion`), **sans couleur ni flash**.
...
### Phase 15.6 — Indicateurs de salaire (médiane + quartiles, style Parcoursup)

**BDD.** La table `stats_sous_populations` est enrichie de **4 colonnes** (`nb_salaires`, `salaire_q1`, `salaire_q2`, `salaire_q3`, toutes `INT NULL`). Salaire mensuel **net en équivalent temps plein** (DSN, méthodologie SIES). Faits vérifiés en BDD : disponible **uniquement à 12/18/24/30 mois** (jamais à 6) ; les 4 colonnes sont **NULL ensemble** sous le seuil de diffusion source (20) — une cellule n'est donc exploitable que si `salaire_q2 IS NOT NULL` ; disponibilité **indépendante de `nb_sortants`** (une mention à `nb_sortants` ≥ 20 peut n'avoir aucun salaire à une durée donnée — couverture DSN partielle) ; **pas de données BUT** à ce jour (recul temporel insuffisant). **Quantiles non agrégeables** : pas de somme possible → aucune valeur en mode agrégat établissement.

**API.**
- `/quadrant/details` expose `salaires { disponible, donnees_par_millesime_et_duree }` pour la mention **référence** (diplômé/ensemble/français/ensemble), sur tous les millésimes × les 4 durées de salaire. Calculé **uniquement pour une mention unique** (vue Mentions, ou vue Positionnement avec filtre mention) ; en mode agrégat établissement → `disponible=false` (quantiles non agrégeables). Helper `chargerSalairesReference`. Objet vide sérialisé en `{}` (`stdClass`).
- `/analyse-sous-populations` expose `salaires_par_sous_pop` (clé = id de sous-pop : `reference`, `femmes`, `hommes`, `apprentis`, `tous_nationalite`, `ensemble_diplomation`), chacun avec `donnees_par_duree { "12": {nb_salaires,q1,q2,q3}|null, "18":…, "24":…, "30":… }` pour le millésime de la modale. **null en mode établissement**. Colonnes salaire ajoutées au SELECT du mode mention uniquement. Helpers `construireSalairesParSousPop` / `salaireCellule`.

**Composants réutilisables (`src/components/sous-populations/`).**
- `BlocSalaireParcoursup.jsx` — bloc « médiane mise en avant + fourchette Q1–Q3 décorative » (props `donnees`, `titre`, `sous_titre`, `taille` `compact`|`standard`, `messageVide`). La légende des bornes est libellée en **quartiles** (« 1er quartile · 25 % gagnent moins » / « 3e quartile · 25 % gagnent plus ») — correction volontaire vs le mockup du brief qui indiquait « 50 % gagnent moins/plus » (qui décrit la médiane, pas les quartiles).
- `GraphiqueEvolutionSalaires.jsx` — SVG 3 courbes : **Q1 pointillé court** (`4 2`, gris), **Q2 médiane trait plein** (bleu `#0E0E60`, épais), **Q3 pointillé long** (`8 4`, gris). Abscisses `millesimes`|`durees`, `echelle_y` optionnelle (échelle commune), courbes interrompues sur trou. **Tooltip** au survol d'une abscisse (bande invisible pleine hauteur) → `Q1 · Médiane · Q3` du point, via `createPortal` + `useAutoPlacement` + `.graphe-tooltip` (mêmes conventions que `MiniGrapheEvolution`/`GrapheMultiCourbes`). **Légende** Q1/Q2/Q3 sous le graphe (swatches au style de ligne réel). Prop **`variant`** : `compact` (barre latérale, **sans marqueur** — cohérent avec les autres mini-graphes du panneau) vs `standard` (modale, **avec marqueur** anneau sur `marqueur_x` — lisible pendant l'animation).
- Utils `src/utils/salaires.js` : `DUREES_SALAIRE` (12/18/24/30), `formatEuroSalaire`, `donneesVersPoints`, `echelleCommune`, `celluleSalaireValide`, `aAuMoinsUnSalaire`.

**UX — barre latérale (fiche détaillée d'une mention).** Section `SectionSalaires` (`DetailsPanel.jsx`), affichée **seulement si `salaires.disponible`** (sinon absente, pas de placeholder). Placée **en fin de liste des indicateurs** (après « Autres indicateurs », avant le bouton d'analyse fine — Phase 15.6.1). Titre `<h3>` sans style dédié → hérite des petites capitales grises de `.panneau-details h3` (même rendu que « AUTRES INDICATEURS »). Trois blocs : (1) `BlocSalaireParcoursup` compact à la **durée effective** ; (2) évolution **sur les millésimes** (durée fixe) ; (3) évolution **sur la durée** (millésime fixe = courant). Graphiques en `variant="compact"` (**sans marqueur**, avec tooltip + légende). **Règle de durée effective** : durée d'un axe (X prioritaire, sinon Y) → sinon 12 → sinon première durée présente au millésime courant (12→18→24→30) ; le fallback porte **uniquement sur la durée au sein du millésime courant** (pas de bascule cross-millésime → bloc 1 affiche « Non disponible » si le millésime courant est entièrement vide). **Bloc 3 masqué** si le millésime courant n'a aucune donnée de salaire (Phase 15.6.2 : éviter un graphique plat pour un millésime récent sans enquête) — le bloc 2 reste affiché (il montre que les données existent pour d'autres millésimes). **Échelle Y commune** aux blocs 2 et 3 quand les deux sont visibles ; calculée sur les seules données du bloc 2 quand le bloc 3 est masqué. Note de lecture sous les blocs.

**UX — onglet « Salaires » de la modale d'analyse fine.** `OngletSalaires.jsx`, inséré après « Parcours » (avant « Mentions agrégées »), **visible seulement si au moins une sous-pop a des salaires** (donc masqué en mode établissement et pour les BUT). Grille de 6 blocs (référence + 5 sous-pops principales), chacun = `BlocSalaireParcoursup` + `GraphiqueEvolutionSalaires` sur les durées en `variant="standard"` (**avec marqueur**, échelle Y **individuelle**). Libellés des sous-pops alignés sur l'onglet « Comparaison » (`sp.libelle` du backend) : « Diplômés français et étrangers » (effet nationalité), « Diplômés et non diplômés français » (effet diplomation). Sous-pop sans données → bloc conservé avec message (grille préservée). Piloté par le **slider/animation commun** (`dureeCourante`) ; les contrôles ⏮▶⏭ + Vitesse sont **partagés** Quadrant ↔ Salaires (`renderControlesLecture(scope)`), et `changerOnglet` ne met plus en pause en passant sur Salaires. **Cas durée = 6** : bloc Parcoursup « Non disponible à 6 mois », graphique sur 12/18/24/30.

**Conventions de courbe salaire (Phase 15.6.1).** Trois quartiles toujours différenciés par le **style de trait**, identique en barre latérale et en modale : Q1 pointillé court (`4 2`), médiane Q2 trait plein épais (bleu principal), Q3 pointillé long (`8 4`). **Marqueurs** présents en modale (`variant="standard"`, suivent l'animation) mais **absents en barre latérale** (`variant="compact"`, cohérence avec les autres mini-graphes du panneau). **Tooltips** Q1+Q2+Q3 au survol dans les deux contextes.
